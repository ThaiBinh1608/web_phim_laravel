               ...
               <div class="row">
                  <div class="col-xs-6">
                     <div class="yoast_breadcrumb hidden-xs"><span><span>
                        <a href="{{route('category',[$movie->category->slug])}}">{{$movie->category->title}}</a> » <span class="breadcrumb_last" aria-current="page"><a href="">{{$movie->title}}</a> </span></span></span></span></div>
                  </div>
               </div>
            </div>
               ...
               <div class="iframe_phim">
                  ...
                  <div id="halim-list-server">
                     <ul class="nav nav-tabs" role="tablist">
                        @if($movie->phude==0)
                        <li role="presentation" class="active server-1"><a href="#server-0" aria-controls="server-0" role="tab" data-toggle="tab"><i class="hl-server">Vietsub</i>
                        </a></li>
                        @elseif($movie->phude==1)
                        <li role="presentation" class="active server-1"><a href="#server-0" aria-controls="server-0" role="tab" data-toggle="tab"><i class="hl-server">Thuyết minh</i>
                                          ...
                                          <ul class="halim-list-eps">
                                             @foreach ($episode_list as $key=> $epi)
                                                @if($epi->server == $ser->id)
                                                   ...
                                                   <li class="halim-episode"><span class="halim-btn halim-btn-2 {{$tapphim== $epi->episode && $server_active=='server-'.$ser->id ? 'active':''}}"> {{$epi->episode}}</span></li></a>
                                                @endif
                                             @endforeach
                                          </ul>
                                       @endif
